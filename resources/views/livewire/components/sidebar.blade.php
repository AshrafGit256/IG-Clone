<div x-data="{
    ...
    <ul class="space-y-4 mt-2">
        ...
        <li>
            <a wire:navigate href="{{route('profile.home',auth()->user()->username)}}" wire:navigate href="" class="flex items-center gap-5">

                <x-avatar src="https://randomuser.me/api/portraits/men/1.jpg" class="w-10 h-10" />

                <h4 x-cloak x-show="!(shrink||drawer)"
                    class="{{request()->routeIs('profile.home')?'font-bold':'font-medium'}} text-lg ">Profile</h4>
            </a>
        </li>
    ...
    <div x-show="drawer"
        x-cloak x-transition.origin.left
        @click.outside="drawer=false;showSearch=false;showNotifications=false"
        class="fixed inset-y-0 left-[70px] w-120 bg-white px-4 overflow-y-scroll overflow-x-hidden shadow z-[50] border rounded-r-2xl z-[5]">
